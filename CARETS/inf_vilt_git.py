...
error_data = []
## This script is for generating predictions from ViLT or GIT, 
## please change the corresponding comment part and saving path at the end of the script
...
for test_name, split in orig_dataset.splits:
    ## run for the first time, if f'{test_name}.npy' not available under data directory
    ...
    print("Each sample: ", (t_end-t_begin)/total_len)
    print('Error size: ', len(dataset) - total_len)

np.save('preds_git.npy', predictions)
np.save('error_data_git.npy', error_data)
