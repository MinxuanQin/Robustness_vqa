## Some modifications from Minxuan Qin
import json
import warnings
...
class Test:
    ...
    ## MOD Minxuan: add WUPS score
    def total_wups(self, preds):
        score = 0
        assert set(self._questions_dict).issubset(set(preds)), AssertionError(
            'Did not receive predictions for all questions.')
        for question in self.questions:
            q_id = question['question_id']
            ## Actually only one element in the dictionary
            label_dict = question['label']
            label = list(label_dict.keys())[0]
            prediction = preds[q_id]

            ## try to find synsets in wordnet
            syn_label = wordnet.synsets(label)
            syn_pred = wordnet.synsets(prediction)

            if bool(syn_label) and bool(syn_pred):
                ## pick first meaning as feature vector to compute
                syn_label = syn_label[0]
                syn_pred = syn_pred[0]
                score += syn_label.wup_similarity(syn_pred)
    ...
    def orig_accuracy(self, preds):
        score = 0
        assert len(self._questions_dict) == len(self.questions)
        orig_questions = {
        ...
        ## MOD Minxuan: fix error for read dictionary and compute avg
        for q_id, question in orig_questions.items():
            score += question['label'].get(preds[q_id], 0)
        return score / len(orig_questions)

    ## MOD Minxuan: add wups for original questions
    def orig_wups(self, preds):
        score = 0
        orig_questions = {
            k: v for k, v in self._questions_dict.items() if not v['perturbed']}
        assert set(orig_questions).issubset(preds), AssertionError(
    ...
    def perturbed_accuracy(self, preds):
        ...
        ## MOD Minxuan: fix error for read dictionary and compute avg
        for q_id, question in perturbed_question.items():
            score += question['label'].get(preds[q_id], 0)
        return score / len(perturbed_question)

    ## MOD Minxuan: add wups for original questions
    def perturbed_wups(self, preds):
        score = 0
        perturbed_questions = {
            k: v for k, v in self._questions_dict.items() if v['perturbed']}
        assert set(perturbed_questions).issubset(preds), AssertionError(
            'Did not receive predictions for all non-perturbed questions.')
        for q_id, question in perturbed_questions.items():
            ## Actually only one element in the dictionary
            label_dict = question['label']
            label = list(label_dict.keys())[0]
            prediction = preds[q_id]
        ...
        return score / len(perturbed_questions)
